...
public class BPlusTree {
    ...
    public void put(DataBox key, RecordId rid) {
        ...
        if (res.isPresent()) {
            List<DataBox> keys = Arrays.asList(res.get().getFirst());
            List<Long> children = Arrays.asList(root.getPage().getPageNum(), res.get().getSecond());
    ...
    private class BPlusTreeIterator implements Iterator<RecordId> {
        ...
        Optional<LeafNode> currentNode;
        int offset;

        BPlusTreeIterator(BPlusNode root, Optional<DataBox> key) {
            offset = 0;
            if (!key.isPresent()) {
                currentNode = Optional.of(root.getLeftmostLeaf());
            } else {
                currentNode = Optional.of(root.get(key.get()));
                if (currentNode.isPresent()) {
                    while (currentNode.get().getKeys().get(offset).compareTo(key.get()) < 0) {
                        offset ++;
                    }
                }
            }
        ...
        public boolean hasNext() {
            // TODO(proj2): implement
//            while (currentNode.isPresent() && currentNode.get().getRids().size() <= offset) {
//                offset = 0;
//                currentNode = currentNode.get().getRightSibling();
//            }

            return currentNode.isPresent() && currentNode.get().getRids().size() > offset;
        }

        @Override
        public RecordId next() {
            // TODO(proj2): implement
            RecordId res = currentNode.get().getRids().get(offset);
            offset++;
            if (offset >= currentNode.get().getRids().size()) {
                offset = 0;
                currentNode = currentNode.get().getRightSibling();
            }
